...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { //SOBRESCRITA DO METODO DESSA FORMA ESTAMOS COLOCANDO NOSSA PROPRIA CONFIGURACAO PARAM ESTE METODO
        http.csrf(csrf -> csrf.disable()) //desabilita a proteção contra CSRF (Cross-Site Request Forgery)
                .authorizeHttpRequests(auth -> {
                    auth.requestMatchers("/candidate/").permitAll()
                            .requestMatchers("/company/").permitAll().requestMatchers("/auth/company").permitAll();
